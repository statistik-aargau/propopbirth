...
Ihe original FSO model uses an **ARIMA model** for the trend period.
Depending on the structure of the past TFR and MAB data, the ARIMA
forecast is not always stable. Therefore, in
[propopbirth](https://github.com/statistik-aargau/propopbirth) a
**linear model** (`lm`) can be used as an alternative to the ARIMA
model.
...
[propopbirth](https://github.com/statistik-aargau/propopbirth) an
...
temporal_end_tfr <- tidyr::expand_grid(
  spatial_unit = c("Aarau", "Stadt Zürich", "Uster"),
  nat = c("ch", "int")
) |>
  dplyr::mutate(y_end = c(0.8, 1.5, 0.9, 0.9, 0.8, 2.0))
```
