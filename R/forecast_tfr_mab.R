...
trend_dat <- trend_lm(input_lm = input_dat,
...
# start and end point
points_dat <- temporal_points(input_past = input_dat,
...
temporal_dat <- temporal_cubic(points_dat = points_dat,
               year_begin = temporal_model[2],
               year_end = temporal_model[3]) 

} else if (temporal_model[1] == "Bezier"){

temporal_dat <- temporal_Bezier(points_dat = points_dat,
               year_begin = temporal_model[2],
               year_end = temporal_model[3])

} else if (temporal_model[1] == "constant"){

temporal_dat <- temporal_constant(points_dat = points_dat,
               year_begin = temporal_model[2],
               year_end = temporal_model[3])
...
constant_dat <- constant_model(in_dat = temporal_dat,
